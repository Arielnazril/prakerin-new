...
<div class="max-w-5xl mx-auto space-y-8 select-none pb-12 antialiased">
    ...
    <div class="bg-white rounded-3xl shadow-sm hover:shadow-xl border border-slate-200/80 overflow-hidden relative transition-all duration-300">
        ...
        {{-- HEADER DENGAN GRADIENT PREMIUM HIJAU --}}
        <div class="bg-gradient-to-r from-slate-950 via-emerald-950 to-slate-900 p-6 sm:p-10 text-white relative overflow-hidden group">
            {{-- Light Glow Overlay & Blur Decorative Elements --}}
            <div class="absolute -right-12 -top-12 w-64 h-64 bg-emerald-500/20 rounded-full blur-3xl group-hover:bg-emerald-400/30 transition-all duration-700 pointer-events-none"></div>
            <div class="absolute right-1/3 -bottom-12 w-56 h-56 bg-lime-500/15 rounded-full blur-2xl group-hover:bg-lime-400/25 transition-all duration-700 pointer-events-none"></div>
            
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 relative z-10">
                <div class="space-y-2">
                    <span class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 backdrop-blur-md border border-white/15 text-[10px] font-black tracking-widest uppercase text-emerald-300">
                        <span class="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span> Official Document
                    </span>
                    <h1 class="text-2xl sm:text-3xl lg:text-4xl font-black tracking-tight flex items-center gap-2 text-white">
                ...
                <div class="bg-white/10 backdrop-blur-md rounded-2xl p-4 sm:p-5 border border-white/15 flex items-center gap-4 self-stretch sm:self-auto justify-between sm:justify-start shadow-lg">
                    <div class="text-right">
                        <div class="text-3xl sm:text-4xl font-black tracking-tight font-mono text-lime-400 drop-shadow-sm">
                    ...
                    <div class="w-12 h-12 rounded-2xl bg-lime-400/20 border border-lime-400/30 flex items-center justify-center text-lime-400 shadow-inner shrink-0">
            ...
            <div class="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm border-t border-white/15 pt-6 relative z-10">
                <div class="space-y-1">
                    <p class="text-emerald-200/70 text-[10px] uppercase tracking-widest font-black">Nama Lengkap Siswa</p>
                ...
                <div class="space-y-1">
                    <p class="text-emerald-200/70 text-[10px] uppercase tracking-widest font-black">Instansi / Tempat Magang</p>
                    <p class="font-extrabold text-base sm:text-lg text-white tracking-wide flex items-center gap-2">
                        <i class="fas fa-building text-lime-400"></i>
        ...
        <div class="p-6 sm:p-10 space-y-8">
            <h3 class="font-black text-slate-800 text-base sm:text-lg tracking-tight flex items-center gap-3 border-b border-slate-100 pb-4">
                <div class="bg-emerald-50 p-2.5 rounded-2xl border border-emerald-100 text-emerald-700 shadow-2xs">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="bg-gradient-to-b from-emerald-50/40 via-slate-50/30 to-white p-6 sm:p-7 rounded-3xl border border-emerald-100/80 flex flex-col justify-between shadow-2xs hover:shadow-md transition-all">
                    <div>
                        <h4 class="font-black text-emerald-950 mb-5 flex items-center text-sm tracking-tight bg-emerald-100/70 text-emerald-900 px-3.5 py-2 rounded-xl border border-emerald-200/60 w-max shadow-2xs">
                            <i class="fas fa-building mr-2 text-emerald-700"></i> Penilaian Industri
                        </h4>
                        @if($nilaiMentor)
                            <ul class="space-y-3 text-sm text-slate-700">
                                <li class="flex justify-between items-center bg-white p-3.5 rounded-2xl border border-slate-200/70 shadow-2xs">
                                    <span class="font-semibold text-slate-600 flex items-center gap-2.5 text-xs sm:text-sm">
                                        <i class="fas fa-cog text-xs text-emerald-600"></i> Aspek Teknis (Hard Skill)
                                    </span>
                                ...
                                <li class="flex justify-between items-center bg-white p-3.5 rounded-2xl border border-slate-200/70 shadow-2xs">
                                    <span class="font-semibold text-slate-600 flex items-center gap-2.5 text-xs sm:text-sm">
                                        <i class="fas fa-users text-xs text-emerald-600"></i> Aspek Non-Teknis (Soft Skill)
                                    </span>
                    ...
                    @if($nilaiMentor)
                        <div class="mt-6 flex justify-between items-center pt-4 border-t border-emerald-100/80 font-bold text-emerald-950">
                            <span class="text-xs uppercase tracking-wider font-extrabold text-emerald-900/80">Rata-rata Industri</span>
                            <span class="font-black font-mono bg-gradient-to-r from-emerald-800 to-emerald-600 text-white px-3.5 py-1.5 rounded-xl shadow-md text-sm">{{ $nilaiMentor->nilai_akhir }}</span>
                        </div>
                    @endif
                </div>

                {{-- BLOK PENILAIAN SEKOLAH --}}
                <div class="bg-gradient-to-b from-teal-50/40 via-slate-50/30 to-white p-6 sm:p-7 rounded-3xl border border-teal-100/80 flex flex-col justify-between shadow-2xs hover:shadow-md transition-all">
                    <div>
                        <h4 class="font-black text-teal-950 mb-5 flex items-center text-sm tracking-tight bg-teal-100/70 text-teal-900 px-3.5 py-2 rounded-xl border border-teal-200/60 w-max shadow-2xs">
                            <i class="fas fa-school mr-2 text-teal-700"></i> Penilaian Sekolah
                        </h4>
                        @if($nilaiGuru)
                            <ul class="space-y-3 text-sm text-slate-700">
                                <li class="flex justify-between items-center bg-white p-3.5 rounded-2xl border border-slate-200/70 shadow-2xs">
                                    <span class="font-semibold text-slate-600 flex items-center gap-2.5 text-xs sm:text-sm">
                                        <i class="fas fa-file-alt text-xs text-teal-600"></i> Laporan Tertulis
                                    </span>
                                ...
                                <li class="flex justify-between items-center bg-white p-3.5 rounded-2xl border border-slate-200/70 shadow-2xs">
                                    <span class="font-semibold text-slate-600 flex items-center gap-2.5 text-xs sm:text-sm">
                                        <i class="fas fa-chalkboard-teacher text-xs text-teal-600"></i> Presentasi / Sidang
                                    </span>
                    ...
                    @if($nilaiGuru)
                        <div class="mt-6 flex justify-between items-center pt-4 border-t border-teal-100/80 font-bold text-teal-950">
                            <span class="text-xs uppercase tracking-wider font-extrabold text-teal-900/80">Rata-rata Sekolah</span>
                            <span class="font-black font-mono bg-gradient-to-r from-teal-700 to-emerald-700 text-white px-3.5 py-1.5 rounded-xl shadow-md text-sm">{{ $nilaiGuru->nilai_akhir }}</span>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="bg-slate-50/80 px-6 sm:px-10 py-5 border-t border-slate-100 flex justify-end items-center">
            @if($placement->is_completed)
                <a href="{{ route('siswa.sertifikat.cetak', $placement->id) }}" target="_blank" class="bg-gradient-to-r from-emerald-800 via-emerald-700 to-emerald-900 hover:from-emerald-900 hover:to-slate-900 text-white px-7 py-3.5 rounded-2xl font-black text-xs shadow-md shadow-emerald-900/20 hover:shadow-lg hover:shadow-emerald-900/30 transition duration-200 active:scale-95 flex items-center gap-2.5 cursor-pointer tracking-wider uppercase inline-flex">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {{-- CATATAN MENTOR --}}
        <div class="bg-white p-6 sm:p-7 rounded-3xl shadow-sm border border-slate-200/80 border-l-4 border-l-emerald-600 flex flex-col justify-between transition-all duration-300 hover:shadow-md">
            <div>
                <h4 class="font-black text-slate-800 text-sm mb-4 flex items-center tracking-tight">
                    <i class="fas fa-comment-dots text-emerald-600 mr-2.5 text-base"></i> Catatan Mentor
                </h4>
            ...
            <div class="mt-6 flex items-center bg-emerald-50/50 p-3 rounded-2xl border border-emerald-100/60">
                <div class="w-9 h-9 rounded-xl bg-gradient-to-tr from-emerald-800 to-emerald-600 flex items-center justify-center text-white font-black text-xs shadow-md shrink-0">
        ...
        {{-- EVALUASI GURU --}}
        <div class="bg-white p-6 sm:p-7 rounded-3xl shadow-sm border border-slate-200/80 border-l-4 border-l-teal-600 flex flex-col justify-between transition-all duration-300 hover:shadow-md">
            <div>
                <h4 class="font-black text-slate-800 text-sm mb-4 flex items-center tracking-tight">
                    <i class="fas fa-quote-left text-teal-600 mr-2.5 text-base"></i> Evaluasi Guru
                </h4>
                <div class="bg-slate-50 p-4 sm:p-5 rounded-2xl italic text-slate-600 text-xs sm:text-sm leading-relaxed font-medium border border-slate-200/60 relative">
                    <span class="text-4xl text-slate-200 absolute right-3 bottom-0 font-serif select-none pointer-events-none">”</span>
            ...
            <div class="mt-6 flex items-center bg-teal-50/50 p-3 rounded-2xl border border-teal-100/60">
                <div class="w-9 h-9 rounded-xl bg-gradient-to-tr from-teal-800 to-emerald-600 flex items-center justify-center text-white font-black text-xs shadow-md shrink-0">
