...
{{-- BANNER WELCOME --}}
<div class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-950 p-6 sm:p-8 text-white shadow-2xl shadow-emerald-950/20 border border-slate-700/50 transition-all duration-500 hover:shadow-emerald-900/30 group mb-8">
    {{-- Light Glow Overlay & Decorative Blur --}}
    <div class="absolute -right-12 -top-12 w-72 h-72 bg-emerald-500/20 rounded-full blur-3xl group-hover:bg-emerald-400/30 transition-all duration-700 pointer-events-none"></div>
    <div class="absolute -left-12 -bottom-12 w-64 h-64 bg-teal-500/20 rounded-full blur-3xl group-hover:bg-teal-400/30 transition-all duration-700 pointer-events-none"></div>
    ...
    <div class="relative z-10 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        ...
        <div class="flex items-center gap-5 w-full lg:w-auto">
            ...
            <div class="hidden sm:flex shrink-0 h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-tr from-emerald-600 via-teal-600 to-emerald-400 text-white text-2xl font-black shadow-lg shadow-emerald-500/25 border border-white/20 transition-all duration-300 transform group-hover:scale-105 group-hover:rotate-2">
                {{ strtoupper(substr(Auth::user()->name ?? 'M', 0, 1)) }}
            </div>

            <div class="space-y-2 max-w-2xl">
                {{-- Status Badge --}}
                <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-xs font-extrabold text-emerald-300 tracking-wider uppercase shadow-inner">
                ...
                <h2 class="text-2xl sm:text-3xl lg:text-4xl font-black tracking-tight flex flex-wrap items-center gap-2.5">
                    Selamat Datang, 
                    <span class="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 via-teal-300 to-emerald-200 font-black tracking-wide">
                        {{ Auth::user()->name }}
                    </span>
                ...
                <p class="text-slate-300 text-sm sm:text-base font-medium leading-relaxed flex flex-wrap items-center gap-2">
                    ...
                    <span class="inline-flex items-center px-3 py-1 rounded-xl bg-white/10 backdrop-blur-md border border-white/15 text-xs font-bold text-teal-200 shadow-sm transition-colors duration-300 group-hover:border-emerald-400/50">
                        <i class="fas fa-building mr-1.5 text-emerald-400 text-xs"></i>
        ...
        <div class="relative z-10 bg-slate-800/60 backdrop-blur-xl px-5 py-3.5 rounded-2xl border border-white/15 text-white text-sm font-bold shadow-xl flex items-center shrink-0 gap-4 w-full sm:w-auto justify-between sm:justify-start">
            ...
            <div class="flex items-center border-r border-white/20 pr-4 text-slate-200">
                <i class="far fa-calendar-alt mr-2.5 text-emerald-400 text-base animate-pulse"></i> 
            ...
            {{-- Jam Real-Time --}}
            <div class="flex items-center pl-1 font-mono tracking-wider bg-gradient-to-r from-emerald-600 to-teal-600 text-white px-3.5 py-1.5 rounded-xl text-xs font-black shadow-md border border-emerald-400/30 group-hover:scale-105 transition-transform">
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
    ...
    {{-- CARD 1: TOTAL SISWA (Gradient Emerald Ultra Modern) --}}
    <div class="bg-gradient-to-br from-emerald-600 via-teal-600 to-emerald-800 text-white p-6 rounded-3xl shadow-xl shadow-emerald-500/10 hover:shadow-2xl hover:shadow-emerald-500/25 hover:-translate-y-1.5 transition-all duration-300 group relative overflow-hidden border border-emerald-400/30 flex flex-col justify-between">
        ...
        <div class="flex items-start justify-between relative z-10">
            ...
            <div class="p-3.5 bg-white/15 backdrop-blur-md text-white rounded-2xl border border-white/25 shadow-lg flex items-center justify-center group-hover:scale-110 group-hover:rotate-3 transition-all duration-300">
                <i class="fas fa-users text-2xl text-emerald-100"></i>
            </div>
        </div>
        ...
        <div class="mt-6 pt-3 border-t border-white/15 relative z-10 flex items-center justify-between text-[11px] font-extrabold text-emerald-100/90 tracking-wide">
            <span class="flex items-center gap-1.5">
                <i class="fas fa-user-check text-emerald-300"></i> Siswa Terdaftar
            </span>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
    ...
    {{-- CARD UTAMA LOKASI MAGANG (Background Dark Emerald Slate) --}}
    <div class="relative overflow-hidden rounded-2xl border border-slate-700 bg-gradient-to-br from-slate-900 via-slate-800 to-emerald-950 text-white shadow-xl transition-all duration-300 hover:shadow-2xl lg:col-span-3 group">
        <div class="absolute -right-6 -top-6 w-32 h-32 rounded-full bg-emerald-500/10 blur-2xl opacity-0 transition-opacity duration-500 group-hover:opacity-100 pointer-events-none"></div>
        ...
        <div class="flex justify-between items-center px-6 py-4 border-b border-slate-700/60 bg-black/20">
            <h3 class="flex items-center font-bold tracking-tight text-sm sm:text-base text-slate-100">
                <i class="fas fa-building mr-2.5 text-emerald-400 text-lg"></i> 
                Lokasi Magang Perusahaan
            </h3>
            <span class="inline-flex items-center px-2.5 py-1 rounded-full bg-emerald-500/20 border border-emerald-400/30 text-[11px] font-extrabold text-emerald-300 uppercase tracking-wider">
        ...
        <div class="p-6">
            <div class="flex flex-col sm:flex-row items-start gap-5">
                {{-- Box Ikon Besar --}}
                <div class="flex shrink-0 h-20 w-20 items-center justify-center rounded-2xl border border-white/15 bg-white/10 backdrop-blur-md text-teal-300 shadow-inner transition-all duration-300 group-hover:scale-105">
                ...
                <div class="flex-1 space-y-3 min-w-0">
                    <div>
                        <h2 class="text-xl font-black tracking-tight text-white transition-colors duration-200 group-hover:text-teal-300">
                    ...
                    <div class="flex flex-wrap items-center gap-2.5 pt-1 text-xs font-bold text-slate-200">
                        <div class="flex items-center px-3.5 py-1.5 rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm hover:border-emerald-400/50">
                            <i class="fas fa-envelope mr-2.5 w-4 text-center text-emerald-400 text-xs"></i> 
                            <span class="truncate">{{ Auth::user()->instansi->email_perusahaan ?? '-' }}</span>
                        </div>
                        <div class="flex items-center px-3.5 py-1.5 rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm hover:border-teal-400/50">
                            <i class="fas fa-phone mr-2.5 w-4 text-center text-teal-400 text-xs"></i> 
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
    ...
    {{-- CARD BARU 1: AKSES CEPAT MENTOR (Background Dark Emerald/Teal) --}}
    <div class="relative overflow-hidden rounded-2xl border border-emerald-800/50 bg-gradient-to-br from-emerald-950 via-slate-900 to-teal-950 text-white shadow-xl transition-all duration-300 hover:shadow-2xl group">
        <div class="px-6 py-4 border-b border-emerald-700/40 bg-black/20 flex items-center justify-between">
            <h3 class="flex items-center font-bold tracking-tight text-sm sm:text-base text-emerald-200">
        ...
        <div class="p-6">
            <p class="text-xs text-emerald-200/80 font-medium mb-4">Akses langsung ke menu bimbingan dan evaluasi siswa magang secara efisien.</p>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <a href="{{ route('industri.validasi.index') }}" class="flex items-center p-3 rounded-xl border border-amber-400/30 bg-amber-500/10 backdrop-blur-md hover:bg-amber-500/20 hover:border-amber-400/60 transition-all duration-200 group/btn">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="text-[10px] text-emerald-200/70 truncate">Periksa & setujui jurnal</p>
                    </div>
                </a>

                <a href="{{ route('industri.penilaian.index') }}" class="flex items-center p-3 rounded-xl border border-emerald-400/30 bg-emerald-500/10 backdrop-blur-md hover:bg-emerald-500/20 hover:border-emerald-400/60 transition-all duration-200 group/btn">
                    <div class="h-10 w-10 rounded-xl bg-emerald-400 text-slate-950 font-black flex items-center justify-center shrink-0 mr-3 group-hover/btn:scale-105 transition-transform">
                        <i class="fas fa-star text-sm"></i>
                    </div>
                    <div class="min-w-0">
                        <p class="text-xs font-black text-emerald-200 truncate">Input Penilaian</p>
                        <p class="text-[10px] text-emerald-200/70 truncate">Evaluasi kinerja siswa</p>
                    </div>
                </a>
            </div>
        </div>
    ...
    {{-- CARD BARU 2: RINGKASAN PROGRESS & STATUS BIMBINGAN (Background Soft Emerald) --}}
    <div class="relative overflow-hidden rounded-2xl border border-emerald-200 bg-gradient-to-br from-emerald-50 via-teal-50 to-emerald-100/50 shadow-md transition-all duration-300 hover:shadow-xl group">
        <div class="px-6 py-4 border-b border-emerald-100 bg-white/60 backdrop-blur-md flex items-center justify-between">
            <h3 class="flex items-center text-emerald-950 font-bold tracking-tight text-sm sm:text-base">
                <i class="fas fa-chart-line mr-2.5 text-emerald-600 text-lg"></i> 
                Ringkasan Penuntasan Jurnal
            </h3>
            <span class="text-[10px] font-extrabold uppercase px-2 py-0.5 rounded-md bg-emerald-600 text-white shadow-xs">Statistik</span>
        </div>
        <div class="p-6">
            @php
            ...
            <div class="space-y-4">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs font-extrabold text-emerald-950">Persentase Logbook Tervalidasi</p>
                        <p class="text-[11px] text-emerald-700 font-medium">{{ $validatedCount }} dari {{ $totalLogbook ?? 0 }} total aktivitas telah disetujui</p>
                    </div>
                    <span class="text-xl font-black text-emerald-700 font-mono">{{ $percentage }}%</span>
                </div>
                
                {{-- Progress Bar Visual --}}
                <div class="w-full h-3.5 bg-emerald-200/70 rounded-full overflow-hidden p-0.5 border border-emerald-300/50">
                    <div class="h-full bg-gradient-to-r from-emerald-600 via-teal-600 to-emerald-500 rounded-full transition-all duration-1000 shadow-sm" style="width: {{ $percentage }}%"></div>
                </div>

                <div class="grid grid-cols-2 gap-2 pt-1 text-center">
                    <div class="p-2.5 rounded-xl bg-white/80 border border-emerald-100 shadow-xs">
                    ...
                    <div class="p-2.5 rounded-xl bg-white/80 border border-emerald-100 shadow-xs">
                        ...
                        <span class="text-sm font-black text-emerald-700">{{ $totalSiswaCount }} Orang</span>
                    </div>
                </div>
            </div>
        </div>
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
    ...
    {{-- KARTU: DAFTAR SISWA SUDAH DINILAI (Background Teal Soft Gradient) --}}
    <div class="relative overflow-hidden rounded-2xl border border-teal-200 bg-gradient-to-br from-teal-50/90 via-slate-50 to-emerald-50/50 shadow-md transition-all duration-300 hover:shadow-lg group">
        <div class="flex justify-between items-center px-6 py-4 border-b border-teal-200/60 bg-white/70 backdrop-blur-md">
            <h3 class="flex items-center text-teal-950 font-black tracking-tight text-sm sm:text-base">
                <i class="fas fa-star mr-2.5 text-teal-600 text-lg"></i> 
                STATUS PENILAIAN
            </h3>
            <span class="inline-flex items-center px-2 py-0.5 rounded-md bg-teal-600 text-[10px] font-bold text-white uppercase tracking-wider">
                Evaluasi
            </span>
        </div>
        ...
        {{-- Konten Utama --}}
        <div class="p-6">
            <p class="mb-4 text-[10px] font-black tracking-widest text-teal-700 uppercase">
            ...
            <div class="space-y-3 max-h-[320px] overflow-y-auto pr-1 custom-scrollbar">
                ...
                @if(isset($placements) && count($placements) > 0)
                    @foreach($placements as $placement)
                        ...
                        @if($nilaiEvaluasi)
                            ...
                            @endphp
                            <div class="flex items-center justify-between p-3.5 rounded-xl border border-teal-100 bg-white/90 backdrop-blur-sm transition-all duration-300 hover:border-teal-300 hover:shadow-xs group/item">
                                <div class="flex items-center min-w-0 flex-1">
                                    {{-- Profil Huruf Inisial Siswa --}}
                                    <div class="flex shrink-0 h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-tr from-teal-600 to-emerald-600 text-white font-black text-sm shadow-sm transition-all duration-300 group-hover/item:scale-105">
                                        {{ $initialEvaluasi }}
                                    </div>
                                    
                                    {{-- Nama & Identitas Siswa --}}
                                    <div class="min-w-0 flex-1 pl-3.5">
                                        <p class="font-bold text-sm text-slate-800 group-hover/item:text-teal-900 transition-colors duration-200 truncate">
                                            {{ $namaEvaluasiSiswa }}
                                        </p>
                                ...
                                <div class="ml-4 shrink-0">
                                    <span class="inline-flex items-center px-3 py-1.5 rounded-xl font-mono text-xs font-black bg-teal-600 text-white shadow-xs group-hover/item:bg-emerald-700 transition-all duration-300">
                ...
                @if(!$hasEvaluated)
                    <div class="flex flex-col items-center justify-center py-10 px-4 text-center rounded-2xl border border-dashed border-teal-200 bg-white/50 relative overflow-hidden">
                        <div class="h-14 w-14 rounded-2xl bg-white border border-teal-100 shadow-xs flex items-center justify-center text-teal-400 mb-3.5">
                            <i class="fas fa-graduation-cap text-2xl text-teal-300 animate-pulse"></i>
                        </div>
                        <h4 class="text-xs font-extrabold text-teal-900 tracking-tight">Belum Ada Evaluasi</h4>
                        <p class="text-[11px] font-medium text-teal-700/60 mt-1 max-w-[200px] mx-auto leading-normal">
