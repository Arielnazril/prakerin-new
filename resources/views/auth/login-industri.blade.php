...
<body class="bg-gradient-to-br from-slate-950 via-slate-900 to-blue-950 min-h-screen flex items-center justify-center font-sans p-4 relative overflow-hidden selection:bg-blue-600 selection:text-white">
    ...
    <div class="w-full max-w-md bg-white/[0.97] backdrop-blur-xl rounded-[2.5rem] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.4)] p-8 sm:p-10 border border-white/40 relative group overflow-hidden transition-all duration-500 hover:shadow-[0_35px_70px_-10px_rgba(37,99,235,0.15)]">
        ...
        <div class="flex justify-center mb-6 mt-2">
            <div class="w-24 h-24 rounded-3xl bg-white shadow-xl shadow-blue-100/40 flex items-center justify-center border border-slate-100 transform group-hover:scale-105 group-hover:rotate-1 transition-all duration-500 ring-4 ring-slate-50/50">
                <img src="{{ asset('img/logo_smk.png') }}" alt="Logo SMK" class="w-16 h-16 object-contain animate-float">
            </div>
        </div>

        <div class="text-center mb-8">
            <h2 class="text-2xl font-extrabold text-slate-900 tracking-tight sm:text-3xl bg-gradient-to-r from-slate-900 to-slate-700 bg-clip-text text-transparent">Login Industri</h2>
            <div class="flex items-center justify-center gap-3 mt-3">
                <span class="h-[2px] w-5 bg-gradient-to-r from-transparent to-blue-500/50 rounded"></span>
                <p class="text-xs text-slate-500 font-bold uppercase tracking-widest">Pembimbing Lapangan</p>
        ...
        <form action="{{ route('login.store') }}" method="POST" class="space-y-6">
            @csrf
            <div>
                <label class="block text-xs font-bold text-slate-700 uppercase tracking-widest mb-2.5 ml-1">Username / ID Industri</label>
                <div class="relative group/input">
                    <!-- Penambahan Ikon SVG Gedung/Instansi Industri -->
            ...
            <div class="pt-2">
                <button type="submit" 
                    ...
                    <!-- Efek kilatan cahaya halus (shimmer) saat tombol didekati -->
