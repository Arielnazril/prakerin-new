...
<head>
    ...
    <style>
        ...
        .focus-glow:focus {
            box-shadow: 0 0 0 4px rgba(137, 199, 74, 0.2);
        }
...
<body class="bg-slate-950 text-slate-800 antialiased min-h-screen flex flex-col font-sans selection:bg-greenDark selection:text-white relative overflow-x-hidden">
    ...
    <div class="flex-1 flex items-center justify-center py-6 sm:py-16 px-4 sm:px-6 lg:px-8 relative z-10">
        <div class="w-full max-w-5xl animate-fade-in-up">

            <div class="flex flex-col lg:flex-row w-full bg-white/95 backdrop-blur-2xl rounded-3xl sm:rounded-3xl shadow-2xl shadow-slate-950/40 border border-white/20 overflow-hidden transition-all duration-300 hover:shadow-emerald-950/20 my-auto">

                <!-- KIRI: BRANDING HERO SIDE (Informasi Visual Style Gelap Premium) -->
                <div class="hidden lg:flex lg:w-5/12 bg-gradient-to-br from-slate-950 via-emerald-950 to-slate-950 text-white flex-col justify-between p-8 lg:p-10 relative overflow-hidden border-r border-slate-800/80">
                    ...
                    <div class="relative z-10">
                        ...
                        <h2 class="text-2xl lg:text-3xl font-black text-center mb-3 leading-snug tracking-tight bg-clip-text text-transparent bg-gradient-to-b from-white via-emerald-50 to-greenLight">
                            Portal Login
                        </h2>
                    ...
                    <div class="relative z-10 my-4 space-y-2.5 bg-white/5 p-4 rounded-2xl border border-white/10 backdrop-blur-md shadow-inner animate-float">
                        <div class="flex items-center space-x-3 text-xs text-emerald-100 font-semibold">
                            <div class="w-5 h-5 rounded-lg bg-greenLight/20 flex items-center justify-center border border-greenLight/30 shrink-0">
                                <i class="fas fa-shield-alt text-greenLight text-[9px]"></i>
                            </div>
                            <span>Akses Aman & Terenkripsi</span>
                        </div>
                        <div class="flex items-center space-x-3 text-xs text-emerald-100 font-semibold">
                            <div class="w-5 h-5 rounded-lg bg-greenLight/20 flex items-center justify-center border border-greenLight/30 shrink-0">
                                <i class="fas fa-bolt text-greenLight text-[9px]"></i>
                            </div>
                ...
                <div class="w-full lg:w-7/12 p-5 sm:p-10 lg:p-12 relative bg-white/90 backdrop-blur-xl flex flex-col justify-center">

                    <!-- Mobile Branding Bar (Tampil Khusus Layar Kecil) -->
                    <div class="flex lg:hidden items-center justify-between mb-5 pb-3 border-b border-slate-100">
                        ...
                        <span class="text-[9px] sm:text-[10px] font-extrabold uppercase bg-emerald-50 text-greenDark px-2.5 py-1 rounded-full border border-emerald-200/60">Masuk Siswa</span>
                    </div>

                    <div class="w-full max-w-md mx-auto">
                        <div class="text-left mb-5 sm:mb-8">
                            <h2 class="text-xl sm:text-3xl font-black text-slate-900 tracking-tight leading-tight">Selamat Datang!</h2>
                            <p class="text-slate-500 font-medium text-xs sm:text-sm mt-1 leading-normal">Silakan masuk menggunakan akun Siswa Anda.</p>
                        </div>

                        <!-- Alert Status Pending -->
                        @if (session('status_pending'))
                            <div class="mb-5 bg-amber-50/90 border-l-4 border-amber-500 p-3.5 sm:p-4 rounded-2xl shadow-sm border border-amber-200/60 backdrop-blur-sm animate-fadeIn">
                                <div class="flex items-start">
                                    <div class="flex-shrink-0 mt-0.5">
                                        <svg class="h-4 w-4 sm:h-5 sm:w-5 text-amber-500" viewBox="0 0 20 20" fill="currentColor">
                                    ...
                                    <div class="ml-3">
                                        ...
                                        <div class="mt-0.5 text-xs text-amber-800 font-medium leading-relaxed">
                        ...
                        @if (session('status'))
                            <div class="mb-5 bg-emerald-50/90 border-l-4 border-emerald-500 p-3.5 sm:p-4 rounded-2xl shadow-sm border border-emerald-200/60 backdrop-blur-sm animate-fadeIn">
                                <div class="flex items-center">
                                    <div class="flex-shrink-0">
                                        <svg class="h-4 w-4 sm:h-5 sm:w-5 text-emerald-500" viewBox="0 0 20 20" fill="currentColor">
                        ...
                        @if ($errors->any())
                            <div class="mb-5 bg-rose-50/90 border-l-4 border-rose-500 p-3.5 sm:p-4 rounded-2xl shadow-sm border border-rose-200/60 backdrop-blur-sm animate-fadeIn">
                                <div class="flex items-start">
                                    <div class="flex-shrink-0 mt-0.5">
                                        <svg class="h-4 w-4 sm:h-5 sm:w-5 text-rose-500" viewBox="0 0 20 20" fill="currentColor">
                        ...
                        <!-- Form Main -->
                        <form action="{{ route('login.store') }}" method="POST" class="space-y-3.5 sm:space-y-5">
                            @csrf

                            <div>
                                <label class="block text-[10px] sm:text-[11px] font-extrabold text-slate-700 uppercase tracking-wider mb-1.5">NIS (Nomor Induk Siswa)</label>
                                <div class="relative rounded-xl shadow-xs">
                                    ...
                                    <input type="text" name="username" required autofocus
                                        ...
                                        class="w-full pl-10 pr-4 py-2.5 sm:py-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-greenLight/30 focus:border-greenDark outline-none transition duration-200 bg-slate-50/50 focus:bg-white text-xs sm:text-sm placeholder-slate-400 font-medium shadow-xs hover:border-slate-300">
                                </div>
                            </div>

                            <div>
                                <label class="block text-[10px] sm:text-[11px] font-extrabold text-slate-700 uppercase tracking-wider mb-1.5">Kata Sandi</label>
                                <div class="relative rounded-xl shadow-xs">
                                    <span class="absolute inset-y-0 left-0 pl-3.5 flex items-center text-slate-400 pointer-events-none">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3">
                            <a href="{{ route('login.guru') }}"
                                class="w-full py-2.5 sm:py-3 px-4 border-2 border-greenDark rounded-xl font-bold text-greenDark hover:text-white hover:bg-greenDark text-center shadow-xs transition-all duration-300 transform hover:-translate-y-0.5 active:translate-y-0 text-xs tracking-wide flex items-center justify-center">
                                <i class="fas fa-chalkboard-teacher mr-2"></i> LOGIN GURU
                            </a>
                            <a href="{{ route('login.industri') }}"
                                class="w-full py-2.5 sm:py-3 px-4 border-2 border-slate-700 rounded-xl font-bold text-slate-700 hover:text-white hover:bg-slate-800 hover:border-slate-800 text-center shadow-xs transition-all duration-300 transform hover:-translate-y-0.5 active:translate-y-0 text-xs tracking-wide flex items-center justify-center">
                                <i class="fas fa-building mr-2"></i> LOGIN INDUSTRI
                            </a>
                        </div>

                        <!-- Register Link Footer (MODIFIED: Style Card Modern & Eye-Catching) -->
                        <div class="mt-5 sm:mt-8 pt-4 sm:pt-5 border-t border-slate-200/80">
                            <div class="bg-gradient-to-r from-emerald-50/80 via-green-50/50 to-lime-50/80 p-3.5 sm:p-5 rounded-2xl border border-emerald-100/80 text-center shadow-xs">
                                <p class="text-xs sm:text-sm text-slate-600 font-semibold mb-2 sm:mb-2.5">
                                    Belum punya akun e-Prakerin?
                                </p>
                                <a href="{{ route('register') }}"
                                    class="inline-flex items-center justify-center gap-2 w-full py-2.5 px-4 bg-white hover:bg-greenDark text-greenDark hover:text-white font-extrabold text-xs sm:text-sm rounded-xl border border-emerald-200/80 hover:border-greenDark shadow-xs transition-all duration-300 transform hover:-translate-y-0.5 group">
                                    <i class="fas fa-user-plus text-xs text-greenDark group-hover:text-white transition-colors duration-300"></i>
