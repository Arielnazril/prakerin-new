...
{{-- BANNER WELCOME --}}
<div class="relative overflow-hidden rounded-3xl bg-gradient-to-r from-slate-900 via-emerald-950 to-teal-900 p-6 md:p-8 mb-8 text-white shadow-xl shadow-emerald-950/20 border border-slate-800/80 group">
    {{-- Decorative Ornaments --}}
    <div class="absolute -right-12 -top-12 w-56 h-56 bg-emerald-500/20 rounded-full blur-3xl group-hover:bg-emerald-400/30 transition-all duration-700 pointer-events-none"></div>
    ...
    <div class="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div class="flex items-center gap-5">
            ...
            <div class="hidden sm:flex h-16 w-16 rounded-2xl bg-gradient-to-tr from-emerald-500 to-teal-400 text-white font-black items-center justify-center text-2xl shadow-lg shadow-emerald-500/30 border border-white/20 transform group-hover:scale-105 group-hover:rotate-3 transition-all duration-300 shrink-0">
                {{ substr(Auth::user()->name ?? 'G', 0, 1) }}
            </div>
            <div>
                <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 backdrop-blur-md border border-white/10 text-xs font-bold text-emerald-200 mb-2">
        ...
        <div class="w-full md:w-auto bg-white/10 backdrop-blur-md px-4 py-3 rounded-2xl border border-white/15 text-white shadow-inner flex items-center justify-between md:justify-end gap-3 shrink-0">
            <div class="flex items-center border-r border-white/20 pr-3 text-xs md:text-sm text-slate-200 font-semibold">
                <i class="far fa-calendar-alt mr-2 text-emerald-400 text-base"></i> 
                <span id="realtime-date">{{ \Carbon\Carbon::now()->isoFormat('dddd, D MMMM Y') }}</span>
            </div>
            <div class="flex items-center font-mono tracking-wider bg-emerald-600/80 text-white px-3 py-1.5 rounded-xl text-xs shadow-md border border-emerald-400/30 font-black">
                <i class="far fa-clock mr-2 text-xs animate-pulse text-emerald-200"></i>
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    ...
    <div class="bg-white p-6 rounded-3xl shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 border border-slate-200/80 group relative overflow-hidden flex flex-col justify-between h-full">
        ...
        <div class="absolute -right-6 -top-6 w-24 h-24 bg-emerald-50/80 rounded-full blur-xl group-hover:bg-emerald-100 group-hover:scale-125 transition-all duration-500 pointer-events-none"></div>
        ...
        <div class="flex items-start justify-between relative z-10 gap-3">
            <div class="space-y-1.5 min-w-0 flex-1">
                <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider bg-emerald-50 text-emerald-700 border border-emerald-100">
                    Pembimbing Prakerin
                </span>
                <h3 class="text-base font-extrabold text-slate-800 tracking-tight group-hover:text-emerald-600 transition-colors duration-300 truncate mt-1" title="{{ Auth::user()->name ?? 'Guru Bimbingan' }}">
                    {{ Auth::user()->name ?? 'Guru Bimbingan' }}
                </h3>
            </div>
            <div class="p-3.5 bg-emerald-50 text-emerald-600 rounded-2xl border border-emerald-100/60 group-hover:bg-gradient-to-tr group-hover:from-emerald-600 group-hover:to-teal-500 group-hover:text-white group-hover:shadow-lg group-hover:shadow-emerald-500/30 group-hover:rotate-6 transition-all duration-300 shrink-0">
        ...
        <div class="mt-6 pt-3.5 border-t border-slate-100 flex items-center justify-between relative z-10">
            <div class="flex items-center gap-1.5 text-[10px] text-slate-400 font-bold tracking-wider uppercase">
                <i class="fas fa-shield-alt text-emerald-500 text-xs animate-pulse"></i> Hak Akses Guru
            </div>
    ...
    <div class="bg-white p-6 rounded-3xl shadow-sm hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 border border-slate-200/80 group relative overflow-hidden flex flex-col justify-between">
        <div class="absolute right-0 top-0 h-20 w-20 bg-teal-50/50 rounded-bl-full pointer-events-none transition-all duration-500 group-hover:scale-125"></div>
        <div class="absolute left-0 top-0 w-1.5 h-full bg-teal-600 rounded-l-3xl"></div>

        <div class="flex items-center justify-between relative z-10">
            <div class="space-y-1">
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Siswa Bimbingan</p>
                <h3 class="text-3xl md:text-4xl font-black text-slate-800 tracking-tight font-mono group-hover:text-teal-600 transition-colors duration-300">{{ $totalSiswa }}</h3>
            </div>
            <div class="p-3.5 bg-teal-50 text-teal-600 rounded-2xl border border-teal-100/60 group-hover:bg-gradient-to-tr group-hover:from-teal-600 group-hover:to-emerald-500 group-hover:text-white group-hover:shadow-lg group-hover:shadow-teal-500/30 transition-all duration-300">
                <i class="fas fa-user-graduate text-2xl"></i>
            </div>
        </div>
        <div class="mt-6 pt-3.5 border-t border-slate-100 flex items-center gap-1.5 text-[11px] text-slate-400 font-medium">
            <i class="fas fa-info-circle text-teal-500"></i> Terdaftar aktif sistem
        </div>
    </div>
...
<div class="bg-white rounded-3xl shadow-sm border border-slate-200/80 overflow-hidden mb-8 transition-all duration-300 hover:shadow-md">
    <div class="px-6 py-5 border-b border-slate-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-gradient-to-r from-slate-50/80 via-white to-slate-50/30">
        <h3 class="font-black text-slate-800 text-base tracking-tight flex items-center">
            <span class="p-2 rounded-xl bg-emerald-50 text-emerald-600 mr-3 border border-emerald-100">
                ...
                <tr class="hover:bg-slate-50/80 transition duration-150 group">
                    <td class="px-6 py-4 font-extrabold text-slate-800 whitespace-nowrap tracking-wide">
                        <div class="flex items-center gap-3">
                            <div class="h-10 w-10 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-600 text-white font-black flex items-center justify-center text-sm shadow-md shadow-emerald-500/10 border border-emerald-400/20 transform group-hover:scale-105 transition-transform duration-200 shrink-0">
                                {{ substr($log->siswa->name, 0, 1) }}
                            </div>
                            <div class="flex flex-col">
                                <span class="group-hover:text-emerald-600 transition-colors duration-150 text-sm font-bold text-slate-800">{{ $log->siswa->name }}</span>
                            </div>
                        </div>
                    </td>
                    ...
                    <td class="px-6 py-4 text-slate-600 max-w-[340px] font-medium text-xs tracking-wide">
                        <div class="bg-slate-50/60 group-hover:bg-emerald-50/40 p-2.5 rounded-xl border border-slate-200/50 group-hover:border-emerald-100 transition-all duration-150 line-clamp-2">
